...
router.post('/', verifyToken, isCajero, async (req, res) => {
  ...
  try {
    ...
    // ===== MODIFICACIÓN TEMPORAL: AUTO-CREAR APERTURA DE CAJA SI NO EXISTE =====
    let aperturaCierreID;
    
    // Intentar obtener apertura de caja actual del usuario
    ...
    if (cajaResult.recordset.length === 0) {
      // NO HAY CAJA ABIERTA - CREAR UNA AUTOMÁTICAMENTE (TEMPORAL)
      console.log('⚠️ No hay caja abierta, creando apertura automática temporal...');
      
      // Obtener la primera caja disponible
      const cajaDisponible = await transaction.request()
        .query('SELECT TOP 1 CajaID FROM Cajas WHERE Activo = 1');
      
      if (cajaDisponible.recordset.length === 0) {
        await transaction.rollback();
        return res.status(400).json({ 
          success: false, 
          message: 'No hay cajas disponibles en el sistema' 
        });
      }

      const cajaID = cajaDisponible.recordset[0].CajaID;

      // Crear apertura de caja automática
      const aperturaResult = await transaction.request()
        .input('cajaID', sql.Int, cajaID)
        .input('usuarioID', sql.Int, req.userId)
        .input('montoInicial', sql.Decimal(10, 2), 0)
        .query(`
          INSERT INTO AperturaCierreCaja (CajaID, UsuarioID, MontoInicial, Estado)
          VALUES (@cajaID, @usuarioID, @montoInicial, 'ABIERTA');
          SELECT SCOPE_IDENTITY() AS AperturaCierreID;
        `);

      aperturaCierreID = aperturaResult.recordset[0].AperturaCierreID;
      console.log('✅ Apertura de caja creada automáticamente:', aperturaCierreID);
    } else {
      aperturaCierreID = cajaResult.recordset[0].AperturaCierreID;
    }
